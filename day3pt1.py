inp = """................458...689.556..3............197......582........720.........................515..352..286.........670.741.....895.626.......
...
rows: List[List[str]] = [list(i) for i in inp.split('\n')]
...
def extract_number_indecies(row: List[str]) -> List[Tuple[int, int]]:
    num_indecies = []
    prev_char = ""
    start = -1
    for i, char in enumerate(row):
        ...
        if char in "0123456789":
            if start == -1:
                start = i
            if i == len(row) - 1:
                num_indecies.append((start, i))
        else:
            if prev_char in "0123456789":
                end = i - 1
                if start != -1 and end != -1:
                    num_indecies.append((start, end))
                start = -1
...
for index, row in enumerate(rows):
    num_indecies = extract_number_indecies(row)
    for num_index in num_indecies:
        num_locs.append((index, num_index))
    
for possible_part in num_locs:
    prev_row = rows[max(possible_part[0] - 1, 0)] # or current row if first row
    current_row = rows[possible_part[0]]
    next_row = rows[min(possible_part[0] + 1, n_rows - 1)]

    left = max(possible_part[1][0] - 1, 0)
    right = min(possible_part[1][1] + 1, row_length + 1) + 1

    is_adjacent = any(
        any(
            True 
            for item in row[left:right] if item not in "0123456789."
        )
        for row in [prev_row, current_row, next_row]
    )
    if is_adjacent:
        num = int("".join(current_row[possible_part[1][0]:possible_part[1][1]+1]))
        running_total += num
        # print(num)
        # for row in [prev_row, current_row, next_row]:
